    ...
    public function saveCheckoutPaymentMethod($data) {
        ...
        try {
            ...
            if (Mage::getSingleton('checkout/type_onepage')->getQuote()->getPayment()->getMethodInstance()->getCode()=='phoenix_cashondelivery') {
                $fee_v2['payment_name'] = Mage::helper('phoenix_cashondelivery')->__('Cash on Delivery fee');
                $fee_v2['payment_fee'] = Mage::getSingleton('checkout/type_onepage')->getQuote()->getCodTaxAmount() + Mage::getSingleton('checkout/type_onepage')->getQuote()->getCodFee();
            }
            //end editing
